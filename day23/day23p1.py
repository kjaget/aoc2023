...
def add_state(start_state, end_state, path_len, edges):
    if (end_state in edges) and (start_state in edges[end_state]):
        print(f'\tdiscarding')
        return edges

    if start_state not in edges:
        edges[start_state] = {}
    if end_state not in edges:
        edges[end_state] = {}
    if end_state in edges[start_state]:
        edges[start_state][end_state] = max(edges[start_state][end_state], path_len)
    else:
        edges[start_state][end_state] = path_len
    return edges
    ...
    while len(states_to_check) > 0:
        ...
        while len(n) == 1:
            ...
            if end_state.coord == goal:
                n = []
                edges = add_state(start_state, end_state, path_len, edges)
                print(f'goal : start_state: {start_state}, end_state: {end_state}, len: {path_len}, n: {n}')
                print(edges)
            else:
                n = neighbors(tiles, end_state, p1)
            # print(f'\tstart_coord: {start_state}, end_coord: {end_state}, len: {len(coords_seen)}, n: {n}, coords_seen = {coords_seen}')
        # print(f'start_state: {start_state}, end_state: {end_state}, len: {path_len}, n: {n}')
        # if (end_state in edges) and (start_state in edges[end_state]):
        #     print(f'\tdiscarding')
        #     continue
        # if start_state not in edges:
        ...
        for new_state in n:
            ...
            edges = add_state(start_state, end_state, path_len, edges)
            edges = add_state(State(end_state.coord, opposite_directions[esd]), start_state, path_len, edges)
            print(edges)
...
def write_dot(output_filename: str, graph_name: str, edges: dict):
    with open(output_filename, 'w') as f:
        ...
        for e in edges:
            for e2 in edges[e]:
                f.write(f'\t"{e.coord.x}x{e.coord.y}_{e.direction}" -> "{e2.coord.x}x{e2.coord.y}_{e2.direction}" [label={edges[e][e2]}]\n')
        f.write('}')
...
def main():
    ...
    edges = gen_graph(tiles, start_state, end_coord, True)   
    write_dot("p1.dot", "p1", edges)
    p1 = longest_path(start_state, edges)
    print(f'p1 = {p1[State(end_coord, S)]}')
    # print_path(tiles, coords_seen)

    edges = gen_graph(tiles, start_state, end_coord, False)   
    write_dot("p2.dot", "p2", edges)
    p2 = longest_path(start_state, edges)
    print(f'p2 = {p2[State(end_coord, S)]}')
    # print_path(tiles, coords_seen)
